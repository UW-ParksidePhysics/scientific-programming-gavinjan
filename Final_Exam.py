...
def matrix_generator(minimum_x, maximum_x, number_of_dimensions, potential_name, potential_parameter):
    ...
    if potential_name == 'harmonic':
        ...
        reduced_potential = pre_factor * np.sin(wave_vector * (horizontal_grid + 0.5*well_width)) / unit_prefactor
        ...
        reduced_potential /= unit_prefactor
    else:
        horizontal_grid = np.linspace(minimum_x, maximum_x, num=number_of_dimensions)
    ...
    matrix_total = unit_prefactor * (matrix_one + matrix_two + matrix_three)
    return matrix_total
if __name__ == "__main__":
    ...
    for index, potential in enumerate(potential_names):
        hamiltonian_matrix = matrix_generator(x_range[0], x_range[1], dimension_number, potential, length_scale)
        ...
        plt.xlabel(r'$x_i$')
        plt.ylabel(r'$H[x_i]$')
        plt.title(rf'$v =$ {Jankowskipotential}, $N = ${dimension_number}')
        plt.show()
...
for module in modules:
    with open(module+'equations_of_state.py') as module_file:
